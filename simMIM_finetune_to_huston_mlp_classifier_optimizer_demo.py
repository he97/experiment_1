...
from utils import get_virtual_dataset, load_pretrained, to_group
...
def is_equal(params, model):
    if len(params) == 0:
        return
    d = {}
    for name, param in model.named_parameters():
        if params[name].equal(param):
            d[name] = True
        else:
            d[name] = False
        print(f'{name}:{d[name]}')
...
if __name__ == '__main__':
    ...
    if on_mac:
        ...
        source_img = to_group(source_img, config)
    ...
    for iDataSet in range(nDataSet):
        ...
        if on_mac:
            train_dataset, test_dataset = get_virtual_dataset(config, (256, 12, 100), (128, 12, 100))
        ...
        E_optim = build_optimizer(config, model, logger, is_pretrain=False)
