...
from models.net_B import vgg16_net
...
def make_feature(data):
    #data.shape (1833,200,33,33)/(6281,200,33,33)
    num, band, patch_size, _ = data.shape
    data = data.cuda()
    feature = torch.zeros(num, band, 512).cuda()
    model = vgg16_net().cuda()
    for i in range(band):
        in_data = data[:,i,:,:].unsqueeze(1)
